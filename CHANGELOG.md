...
- 缩短 Bing 搜索联想的输入防抖和请求超时，并延迟显示加载状态，减少短请求闪烁、提升输入响应速度。
- 优化搜索联想请求链路，优先通过扩展后台服务获取结果，仅在消息通道异常时回退页面直连。
- 隐藏 Chromium 搜索输入框的原生清除控件，避免与自定义清除按钮同时显示两个关闭图标。

## 1.3.4

- 优化书签页目录栏与内容区滚动条，采用更窄的圆角滑块和与主题协调的青绿色。
- 增加滚动条悬停反馈、隐藏两端箭头，并兼容浅色、深色及 Chromium、Firefox 浏览器。

## 1.3.3

- 修复网站 favicon 与默认书签图标同时显示的问题，网站图标加载成功后完整隐藏默认图标及其底色。
- 将默认图标改为独立回退层，仅在所有 favicon 来源加载失败时显示，并统一覆盖首页、搜索建议和书签选择结果。
